...
struct ibv_qp* RDMA::createQueuePair(struct ibv_pd* pd, struct ibv_cq* send_cq, struct ibv_cq* recv_cq) {
  ...
  queue_pair_init_attr.send_cq = send_cq;         
  queue_pair_init_attr.recv_cq = recv_cq;         
...
void RDMA::post_rdma_write(struct ibv_qp *qp, struct ibv_mr *mr, void *addr, uint32_t length, string r_addr, string r_key)
{
  ...
  struct ibv_send_wr send_wr = {
      ...
      .sg_list    = &sge,
      .num_sge    = 1,
      .opcode     = IBV_WR_RDMA_WRITE_WITH_IMM, //IBV_WR_RDMA_WRITE,
      .send_flags = 0,
      .imm_data   = rand(),
      .wr = {
        .rdma = {
